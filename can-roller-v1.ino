/*
  ...
  - START button (A0):
      * if idle: start run (planned 3s total)
...
// Timing / profile
const uint32_t PLANNED_RUN_MS = 8000; // planned run duration (includes ramps)
...
uint16_t commandedRate() {
  ...
  switch (state) {
    ...
    case CRUISE: {
      ...
      uint32_t cruiseUntil = (PLANNED_RUN_MS > RAMP_MS) ? (PLANNED_RUN_MS - RAMP_MS) : 0;
